...
def agregar_registro():
    codigo = formatear_campo(input("Ingrese el código del aula (máx 10 caracteres): "), 10)
    try:
        largo = float(input("Ingrese el largo: "))
        ancho = float(input("Ingrese el ancho: "))
        capacidad = int(input("Ingrese la capacidad: "))
    except ValueError:
        print("Error: Ingrese valores numéricos válidos para largo, ancho y capacidad.")
        return
    color = formatear_campo(input("Ingrese el color (máx 20 caracteres): "), 20)
    try:
    ...
    # Se arma el registro, se convierten los números a string para concatenar.
    ...
    with open(ARCHIVO_AULAS, "r") as f:
        for linea in f:
            ...
            if linea:
                ...
                print(f"Código: {campos[0].strip()}, Largo: {campos[1]}, Ancho: {campos[2]}, Capacidad: {campos[3]}, Color: {campos[4].strip()}, Tipo: {campos[5]}")
    print("-------------------------\n")

def buscar_registro_por_codigo():
    codigo_buscar = formatear_campo(input("Ingrese el código del aula a buscar: "), 10)
    encontrado = False
    if not os.path.exists(ARCHIVO_AULAS):
        print("No hay registros.")
        return
    with open(ARCHIVO_AULAS, "r") as f:
        for linea in f:
            if linea.startswith(codigo_buscar):
                ...
                print(f"Registro encontrado: Código: {campos[0].strip()}, Largo: {campos[1]}, Ancho: {campos[2]}, Capacidad: {campos[3]}, Color: {campos[4].strip()}, Tipo: {campos[5]}")
                encontrado = True
                break
    if not encontrado:
...
def eliminar_registro():
    codigo_eliminar = formatear_campo(input("Ingrese el código del aula a eliminar: "), 10)
    if not os.path.exists(ARCHIVO_AULAS):
        print("No hay registros.")
        return
...
def actualizar_registro():
    codigo_actualizar = formatear_campo(input("Ingrese el código del aula a actualizar: "), 10)
    if not os.path.exists(ARCHIVO_AULAS):
        print("No hay registros.")
        return
    actualizado = False
    with open(ARCHIVO_AULAS, "r") as f, open("temp.txt", "w") as temp:
        for linea in f:
            if linea.startswith(codigo_actualizar):
                ...
                nuevo_codigo = formatear_campo(input("Nuevo código (máx 10 caracteres): "), 10)
                try:
                    nuevo_largo = float(input("Nuevo largo: "))
                    nuevo_ancho = float(input("Nuevo ancho: "))
                    nueva_capacidad = int(input("Nueva capacidad: "))
                except ValueError:
                    print("Error en la conversión de datos numéricos. Registro no actualizado.")
                    temp.write(linea)
                    continue
                nuevo_color = formatear_campo(input("Nuevo color (máx 20 caracteres): "), 20)
                try:
                    nuevo_tipo = int(input("Nuevo tipo de aula (valor entero corto): "))
                except ValueError:
                    print("Error en la conversión de datos para el tipo. Registro no actualizado.")
                    temp.write(linea)
                    continue
...
def importar_registros():
    ...
    with open(ruta_importar, "r") as f_import, open(ARCHIVO_AULAS, "a") as f_aulas:
        for linea in f_import:
            # Se podría validar la estructura de la línea si es necesario.
            f_aulas.write(linea)
    print("Importación completada.")

def exportar_registros():
    nombre_exportar = input("Ingrese el nombre del archivo de exportación: ")
...
def menu():
    opciones = {
        ...
        "5": ("Buscar registro por código", buscar_registro_por_codigo),
